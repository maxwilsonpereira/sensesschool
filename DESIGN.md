...
Motion follows a **prepared atelier being gently uncovered**. The single authored focal entrance stages the hero thesis, coral underline, authentic photograph, and observation notes over 600-1105ms with the system ease-out curve. Selected evidence groups use one-time, viewport-triggered paper, collage, sequence, or paired reveals; they settle permanently after entry and are intentionally not applied to every section. The manifesto replaces its former generic quote reveal with a single sequential glyph-writing moment: the full sentence occupies its final layout from the start, each glyph uncovers once, and the attribution follows after the sentence completes. Authored entrances and navigation state changes use this deliberately slower cadence, while controls retain their faster 240ms feedback so interaction never feels delayed.
